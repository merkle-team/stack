          ...
          instances.map(async ({ PrivateIpAddress: ip }) => {
            ...
            while (Date.now() - startTime < 120_000) {
              try {
                ...
                const connectResult =
                  await $`ssh -T -vvv -J ${bastionUser}@${bastionHost} -o LogLevel=ERROR -o PasswordAuthentication=no -o BatchMode=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${sshUser}@${ip} bash -s < ${new Response(`
